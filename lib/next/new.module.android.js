...
/**
 * start monitoring for a region with mac address
 *
 * @param {Object: BeaconRegion} region region to monitor (identifier + uuid -> major and minor are optional)
 * @returns {Promise<any>} promise resolves to void or error
 */
function startMonitoringForRegionWithMacAddress(
  region: BeaconRegion,
): Promise<any> {
  return new Promise((resolve, reject) => {
    // NOTE: major and minor are optional values: if user don't assign them we have to send a null value (not undefined):
    BeaconsManager.startMonitoringWithMacAddress(
      region.identifier,
      region.uuid,
...
/**
 * stops monittorings for a region with mac address
 * @param {BeaconRegion} region region (see BeaconRegion type)
 * @returns {Promise<any>} promise resolves to void or error
 */
function stopMonitoringForRegionWithMacAddress(
  region: BeaconRegion,
): Promise<any> {
  return new Promise((resolve, reject) => {
    BeaconsManager.stopMonitoringWithMacAddress(
      region.identifier,
      region.uuid,
      region.bluetoothAddress,
      region.minor ? region.minor : -1,
...
module.exports = {
  ...
  startMonitoringForRegionWithMacAddress,
  startRangingBeaconsInRegion,
  stopMonitoringForRegion,
  stopMonitoringForRegionWithMacAddress,
